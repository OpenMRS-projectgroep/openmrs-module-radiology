...
public class RadiologyOrderServiceImplComponentTest extends BaseModuleContextSensitiveTest {
    ...
    public void shouldCreateRadiologyOrderEncounter() throws Exception {
        ...
        if (patient != null && patient.getPersonName() != null) {
            String patientNaam = patient.getPersonName()
                    .getFullName();
            if (gegenereerdeLogs.contains(patientNaam) && !patientNaam.isEmpty()) {
                fail("BEVEILIGINGSLEK: Echte patiëntnaam '" + patientNaam
                        + "' uit de database is weggeschreven naar de logbestanden!");
            }
        }
        ...
        if (medischeCodePatroon.matcher(gegenereerdeLogs)
                .find()) {
            fail(
                "BEVEILIGINGSLEK: Er is een medische statuscode/diagnosepatroon aangetroffen in de runtime component logs!");
        }
    }
}
